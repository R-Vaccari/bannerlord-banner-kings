...
namespace BannerKings.Settings
{
    public class BannerKingsSettings : AttributeGlobalSettings<BannerKingsSettings>
    {
        ...
        [SettingProperty("{=!}Slower Parties", RequireRestart = false, HintText = "{=!}Slows all parties by 30%. Intended to better reflect a realistic map scale. Default: true.")]
        [SettingPropertyGroup("{=!}Balancing")]
        public bool SlowerParties { get; set; } = true;

        [SettingProperty("{=!}Longer Sieges", RequireRestart = false, HintText = "{=!}Decreases siege camp build speed by 50%. Intended to make sieges harder and more impactful, and prevent multiple sequential sieges of same settlement. Default: true.")]
        [SettingPropertyGroup("{=!}Balancing")]
