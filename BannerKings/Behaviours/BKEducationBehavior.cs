...
namespace BannerKings.Behaviours
{
    public class BKEducationBehavior : CampaignBehaviorBase
    {
        ...
        private void OnPerkOpened(Hero hero, PerkObject perk)
        {
            ...
            {
                InformationManager.ShowInquiry(new InquiryData(new TextObject("{=McOOpC4qW}Double Perks").ToString(),
                    new TextObject("{=506kEi1eh}From now on, double perks will be yielded for the {SKILL} skill. The perks will be rewarded after closing the Character tab with 'Done', not immediatly after selecting them.")
            ...
            else
            {
                ...
                MBInformationManager.AddQuickInformation(new TextObject("{=6EJjdkSFY}You have received the {PERK} as a double perk yield reward.")
        ...
        private void AddDialogue(CampaignGameStarter starter)
        {
            starter.AddPlayerLine("bk_question_preaching", "lord_talk_ask_something_2", "lord_talk_ask_something",
                "{=esHjPZpy7}I would like to buy a book.",
                IsBookSeller,
