...
namespace BannerKings.Behaviours
{
    public class BKEducationBehavior : CampaignBehaviorBase
    {
        ...
        private void OnPerkOpened(Hero hero, PerkObject perk)
        {
            ...
            {
                InformationManager.ShowInquiry(new InquiryData(new TextObject("{=fUsSUF6r5}Double Perks").ToString(),
                    new TextObject("{=OpuTK49ou}From now on, double perks will be yielded for the {SKILL} skill. The perks will be rewarded after closing the Character tab with 'Done', not immediatly after selecting them.")
            ...
            else
            {
                ...
                MBInformationManager.AddQuickInformation(new TextObject("{=hsssNQWhn}You have received the {PERK} as a double perk yield reward.")
        ...
        private void AddDialogue(CampaignGameStarter starter)
        {
            starter.AddPlayerLine("bk_question_preaching", "lord_talk_ask_something_2", "lord_talk_ask_something",
                "{=osuoCnLpa}I would like to buy a book.",
                IsBookSeller,
