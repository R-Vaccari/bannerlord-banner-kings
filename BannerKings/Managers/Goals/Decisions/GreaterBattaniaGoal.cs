...
namespace BannerKings.Managers.Goals.Decisions
{
    internal class GreaterBattaniaGoal : Goal
    {
        ...
        {
            var name = new TextObject("{=EuDXTpKnp}Unite Greater Battania");
            var description = new TextObject("{=KGjc5V1hw}Unite the old Battanian lands back into a greater realm. To the West, the rascal Vlandians have taken the valley of Llyn Modris and called it 'Ocs Hall'. To the East, the bloodthristy Imperials submitted Epicrotea to their domination. The threat of Battanian extermination grows stronger with enemies all around aiming for it's lands. You must bring all battanian and formerly battanian towns and castles under control of your realm.");
        ...
        internal override bool IsFulfilled(out List<TextObject> failedReasons)
        {
            ...
            if (!IsAvailable())
            {
                ...
                var failedReason = new TextObject("{=RWLGjj50h}This title is already founded! de Jure is {DE_JURE.LINK} and de Facto is {DE_FACTO.LINK}.");
            ...
            else
            {
                ...
                {
                    failedReasons.Add(new TextObject("{=Gs3ZMNxZy}You need at least {GOLD}{GOLD_ICON}")
                        .SetTextVariable("GOLD", $"{gold:n0}"));
                }

                if (referenceHero.Clan.Influence < influence)
                {
                    failedReasons.Add(new TextObject("{=6zJKN5uDP}You need at least {INFLUENCE}{INFLUENCE_ICON}")
                ...
                {
                    failedReasons.Add(new TextObject("{=52O4B7R66}You are not part of {CULTURE} culture.")
                ...
                {
                    failedReasons.Add(new TextObject("{=uQCsPBN01}You're not the leader of {KINGDOM}.")
                ...
                else if (referenceHero.Clan.Kingdom != null && referenceHero.Clan.Kingdom.Culture != culture)
                {
                    ...
                    failedReasons.Add(new TextObject("{=pYKK0b6a3}Your kingdom is not part of {CULTURE} culture.")
                        .SetTextVariable("CULTURE", culture.EncyclopediaText));
                }
                else
                {
                    failedReasons.Add(new TextObject("{=vrBdWa4Dm}You are not leader of a kingdom with {CULTURE} culture.")
                        .SetTextVariable("CULTURE", culture.EncyclopediaText));
                }
                ...
                if (religion == null || religion.Faith.GetId() != "amra")
                {
                    ...
                    failedReasons.Add(new TextObject("{=KHRtUwPXt}You do not adhere to the {RELIGION} faith.")
                ...
                (
                    ...
                    select new TextObject("{=0mE8y6xQA}Your kingdom is not de facto ruler of {SETTLEMENT}")
