    ...
    internal class GreaterBattaniaGoal : Goal
    {
        ...
        internal override bool IsFulfilled(out List<TextObject> failedReasons)
        {
            ...
            if (referenceHero.Clan.Kingdom.StringId != "Battania" || !referenceHero.IsKingdomLeader())
            {
                ...
                failedReasons.Add(new TextObject("{!=}You're not the leader of {KINGDOM}").SetTextVariable("KINGDOM", kingdom.EncyclopediaLinkWithName));
            }

            if (settlements.Any(s => s.Owner != referenceHero))
            {
                failedReasons.Add(new TextObject("{=!}You do not own all required fiefs."));
            }

            if (referenceHero.Gold < gold)
            {
                failedReasons.Add(new TextObject("{=!}You need at least {GOLD}{GOLD_ICON}.").SetTextVariable("GOLD", gold));
            }

            if (referenceHero.Clan.Influence < influence)
            {
                failedReasons.Add(new TextObject("{=!}You need at least {INFLUENCE}{INFLUENCE_ICON}.").SetTextVariable("INFLUENCE", influence));
            }
