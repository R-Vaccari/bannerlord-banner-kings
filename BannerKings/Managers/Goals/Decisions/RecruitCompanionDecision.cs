...
namespace BannerKings.Managers.Goals.Decisions
{
    internal class RecruitCompanionDecision : Goal
    {
        ...
        {
            var name = new TextObject("{=kWfxTaEM8}Recruit Companion");
            var description = new TextObject("{=hjFoneGs6}Select a type of companion to recruit.");
        ...
        internal override bool IsFulfilled(out List<TextObject> failedReasons)
        {
            ...
            {
                failedReasons.Add(new TextObject("{=uhqE4Vb3g}You can't afford any companion."));
            }
        ...
        internal override void ShowInquiry()
        {
            ...
            foreach (var companionType in companionTypes)
            {
                ...
                {
                    enabled = false;
                    hint = new TextObject("{=h0T4d84mq}No candidates of this type available.").ToString();
                }
                else if (!enabled)
                {
                    hint = new TextObject("{=0FTcYEBEp} You can't afford the cost:\n{GOLD}{GOLD_ICON}\n{INFLUENCE}{INFLUENCE_ICON}.")
            ...
            MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                new TextObject("{=7GfaygYAG}Companions").ToString(),
                new TextObject("{=p78dxndLD}Choose a companion to recruit.").ToString(),
                options, 
                true, 
                1, 
