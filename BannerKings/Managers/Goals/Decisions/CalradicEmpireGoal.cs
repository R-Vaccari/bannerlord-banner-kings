    ...
    internal class CalradicEmpireGoal : Goal
    {
        ...
        internal override bool IsFulfilled(out List<TextObject> failedReasons)
        {
            ...
            if (!IsAvailable())
            {
                ...
                var failedReason = new TextObject("{=RF3Sj0fVV}This title is already founded! De Jure is {DE_JURE.LINK} and de Facto is {DE_FACTO.LINK}.");
            ...
            else
            {
                ...
                {
                    failedReasons.Add(new TextObject("{=SRj5m7WPH}You need at least {GOLD}{GOLD_ICON}")
                        .SetTextVariable("GOLD", $"{gold:n0}"));
                }

                if (referenceHero.Clan.Influence < influence)
                {
                    failedReasons.Add(new TextObject("{=8LbXFxi45}You need at least {INFLUENCE}{INFLUENCE_ICON}")
                ...
                {
                    failedReasons.Add(new TextObject("{=ChKsrbMjC}You are not part of {CULTURE} culture.")
                ...
                {
                    failedReasons.Add(new TextObject("{=7XPUOf1XU}You're not the leader of an Imperial Kingdom."));
                }
                else if (referenceHero.Clan.Kingdom != null && referenceHero.Clan.Kingdom.Culture != culture)
                {
                    failedReasons.Add(new TextObject("{=BMEK7VSXz}Your kingdom is not part of {CULTURE} culture.")
                        .SetTextVariable("CULTURE", culture.EncyclopediaText));
                }
                else
                {
                    failedReasons.Add(new TextObject("{=A8dgy2cjd}You are not leader of a kingdom with {CULTURE} culture.")
                        .SetTextVariable("CULTURE", culture.EncyclopediaText));
                }
                ...
                if (religion == null || religion.Faith.FaithGroup != DefaultFaiths.Instance.ImperialGroup)
                {
                    ...
                    failedReasons.Add(new TextObject("{=ODxwZ4rRV}You do not adhere to a faith that is part of the {RELIGION} faith group.")
                ...
                (
                    ...
                    select new TextObject("{=kkP5zfAVc}Your kingdom is not de facto ruler of {SETTLEMENT}")
