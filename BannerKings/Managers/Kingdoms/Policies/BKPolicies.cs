...
namespace BannerKings.Managers.Kingdoms.Policies
{
    public class BKPolicies : DefaultTypeInitializer<BKPolicies, PolicyObject>
    {
        ...
        public override void Initialize()
        {
            ...
            LimitedArmyPrivilege.Initialize(new TextObject("{=HMznfMYo}Limited Army Privilege"),
                new TextObject(
                    "{=DdPzyH6r}The privilege of raising armies will be limited to lords of duke level or superior, as well as the crown Marshal."),
                new TextObject("{=yVpKXNd1}limiting the privilege to raise armies"),
                new TextObject(
                    "{=gXhzusbk}Only crown Marhsal or lords with Duke title or higher may create armies\nArmy influence costs increased by 30%\nArmy members receive 50% more influence"),
                0.7f, 0.15f, -0.7f);
        }
    }
}