...
namespace BannerKings.Managers.Kingdoms.Contract
{
    public class BKGenderDecision : BKContractDecision
    {
        ...
        {
            var textObject = new TextObject("{=6hqgjaSV0}Change government to {GOVERNMENT}");
            ...
            bool isShortVersion = false)
        {
            ...
            if (newGovernment)
            {
                textObject = new TextObject("{=HW33L6VTn}The {KINGDOM}'s government is now {POLICY_DESCRIPTION}. {POLICY_SUPPORT}");
            }
            else
            {
                textObject =
                    new TextObject(
                        "{=xu1J16aNv}The {KINGDOM}'s government will continue to be {POLICY_DESCRIPTION}. {POLICY_SUPPORT}");
            }
        ...
        public override TextObject GetSupportDescription()
        {
            var textObject =
                new TextObject(
                    "{=eQTZT4xKC}{CLAN} proposes a change of government, currently {CURRENT} to {PROPOSED}. You can pick your stance regarding this decision.");
        ...
        {
            var textObject = new TextObject("{=8Z2KQA0aO}Vote to change of realm's gender law to {GOVERNMENT}");
            textObject.SetTextVariable("GOVERNMENT", genderLaw.ToString());
            return textObject;
        }
