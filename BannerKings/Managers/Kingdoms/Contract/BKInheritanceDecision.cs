...
namespace BannerKings.Managers.Kingdoms.Contract
{
    public class BKInheritanceDecision : BKContractDecision
    {
        ...
        {
            var textObject = new TextObject("{=LU80uqHfU}Change government to {GOVERNMENT}");
            ...
            bool isShortVersion = false)
        {
            ...
            if (newGovernment)
            {
                textObject = new TextObject("{=aWNZjdUwJ}The {KINGDOM}'s government is now {POLICY_DESCRIPTION}. {POLICY_SUPPORT}");
            }
            else
            {
                textObject =
                    new TextObject(
                        "{=hqy49zO9j}The {KINGDOM}'s government will continue to be {POLICY_DESCRIPTION}. {POLICY_SUPPORT}");
            }
        ...
        public override TextObject GetSupportDescription()
        {
            var textObject =
                new TextObject(
                    "{=AbqjH6Nu2}{CLAN} proposes a change of government, currently {CURRENT} to {PROPOSED}. You can pick your stance regarding this decision.");
        ...
        {
            var textObject = new TextObject("{=1KyzCMMc6}Vote to change realm clans' inheritance to {GOVERNMENT}");
            textObject.SetTextVariable("GOVERNMENT", inheritanceType.ToString());
            return textObject;
        }
