...
namespace BannerKings.Managers.Education.Lifestyles
{
    public class DefaultLifestyles : DefaultTypeInitializer<DefaultLifestyles, Lifestyle>
    {
        ...
        public override void Initialize()
        {
            ...
            Fian.Initialize(new TextObject("{=CRt5bHvww}Fian"), new TextObject("{=eapmu1ARv}"), DefaultSkills.Bow,
                DefaultSkills.TwoHanded,
                new List<PerkObject>
                    {BKPerks.Instance.FianHighlander, BKPerks.Instance.FianRanger, BKPerks.Instance.FianFennid},
                new TextObject(
                    "{=eapmu1ARv}Battanian settlements have +{EFFECT1} militia\nReduced damage by {EFFECT2}% when mounted"),
                1f, 30f,
                cultures.FirstOrDefault(x => x.StringId == "battania"));

            Cataphract = new Lifestyle("lifestyle_cataphract");
            Cataphract.Initialize(new TextObject("{=eapmu1ARv}Cataphract"), new TextObject("{=eapmu1ARv}"),
                ...
                new TextObject("{=eapmu1ARv}Increased renown from victories by {EFFECT1}%\n"),
                0f, 0f,
                cultures.FirstOrDefault(x => x.StringId == "empire"));

            Diplomat = new Lifestyle("lifestyle_diplomat");
            Diplomat.Initialize(new TextObject("{=eapmu1ARv}Diplomat"), new TextObject("{=eapmu1ARv}"),
                DefaultSkills.Charm, BKSkills.Instance.Lordship,
                new List<PerkObject>(),
                new TextObject("{=eapmu1ARv}"), 0f, 0f);

            August = new Lifestyle("lifestyle_august");
            August.Initialize(new TextObject("{=eapmu1ARv}August"), new TextObject("{=eapmu1ARv}"),
                ...
                new TextObject("{=eapmu1ARv}1 knight less is counted towards vassal limit\nTrade penalty increased by {EFFECT2}%"),
                1f, 20f);

            SiegeEngineer = new Lifestyle("lifestyle_siegeEngineer");
            SiegeEngineer.Initialize(new TextObject("{=eapmu1ARv}Siege Engineer"), new TextObject("{=eapmu1ARv}"),
            ...
            CivilAdministrator.Initialize(new TextObject("{=eapmu1ARv}Civil Administrator"), new TextObject("{=eapmu1ARv}"),
                ...
                new TextObject("{=eapmu1ARv}Reduced demesne weight of towns by {EFFECT1}%\nParty size reduced by {EFFECT2}"),
                20f, 8f);

            Caravaneer = new Lifestyle("lifestyle_caravaneer");
            Caravaneer.Initialize(new TextObject("{=eapmu1ARv}Caravaneer"), new TextObject("{=eapmu1ARv}"),
                ...
                new TextObject("{=eapmu1ARv}Reduced trade penalty by {EFFECT1}%\nReduced speed during nighttime by {EFFECT2}%"),
                10f, 8f);

            Artisan = new Lifestyle("lifestyle_artisan");
            Artisan.Initialize(new TextObject("{=eapmu1ARv}Artisan"), new TextObject("{=eapmu1ARv}"),
                DefaultSkills.Crafting, DefaultSkills.Trade, new List<PerkObject> {BKPerks.Instance.ArtisanEntrepeneur},
                new TextObject("{=eapmu1ARv}Chance of botching items when smithing reduced by {EFFECT1}%\n{EFFECT2}%"),
                10f, 8f);

            Outlaw = new Lifestyle("lifestyle_outlaw");
            Outlaw.Initialize(new TextObject("{=eapmu1ARv}Outlaw"), new TextObject("{=eapmu1ARv}"),
            ...
            varyag.Initialize(new TextObject("{=eapmu1ARv}Varyag"), new TextObject("{=eapmu1ARv}"),
                DefaultSkills.Leadership, DefaultSkills.Roguery, new List<PerkObject>(),
                new TextObject(
                    "{=eapmu1ARv}Reduced demesne weight of towns by {EFFECT1}%\nSettlement stability reduced by {EFFECT2}%"),
                20f, 8f);
