...
namespace BannerKings.Managers.Skills
{
    public class BKSkills : DefaultTypeInitializer<BKSkills, SkillObject>
    {
        ...
        public override void Initialize()
        {
            ...
            Scholarship.Initialize(new TextObject("{=SZiy47xSP}Scholarship"),
                    new TextObject("{=EA5ksHVKX}Reading and writting competence as well as knowledge over literary and legal matters."),
            ...
            Theology.Initialize(new TextObject("{=YckKUzqqu}Theology"),
                    new TextObject("{=ZRL6CPn3D}Understanding over spiritual matters. Normally reserved for preachers and the most pious faithful."),
                    SkillObject.SkillTypeEnum.Personal)
                .SetAttribute(BKAttributes.Instance.Wisdom);

            Lordship = Game.Current.ObjectManager.RegisterPresumedObject(new SkillObject("Lordship"));
            Lordship.Initialize(new TextObject("{=oKN54KeUx}Lordship"),
                    new TextObject("{=1RdrJ2eHd}Ability to deal with legal administration of titles and feudal contracts."),
                    SkillObject.SkillTypeEnum.Personal)
                .SetAttribute(BKAttributes.Instance.Wisdom);
        }
