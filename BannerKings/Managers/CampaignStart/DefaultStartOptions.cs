...
namespace BannerKings.Managers.CampaignStart
{
    public class DefaultStartOptions : DefaultTypeInitializer<DefaultStartOptions, StartOption>
    {
        ...
        public override void Initialize()
        {
            ...
            Adventurer.Initialize(new TextObject("{=t6NLTy7eY}Adventurer"),
                new TextObject(
                    "{=CfuB2bbNX}A free spirit, you are roaming the continent without constraints, or a clear objective. The world is for the taking, will you take your share?"),
                new TextObject("{=p0owcNmVt}Vanilla start. No troops, goods or any benefits."),
                1000, 0, 0, 50, 0f,
                null);

            IndebtedLord = new StartOption("start_lord");
            IndebtedLord.Initialize(new TextObject("{=Zt91iesAA}Indebted Lord"),
                new TextObject(
                    "{=nT4S6fBTg}After a series of inherited problems and bad decisions, you find yourself in debt. Thankfuly, you are a landed lord, with income from your Lordship. A food supply and a small retinue accompany you, though their loyalty will be tested by the lack of denars..."),
                new TextObject(
                    "{=vkgFrakBZ}Start as a lord in a kingdom, with a Lordship title. No settlement income or influence for 5 years. The village you own can be managed by you, and you will receive it's income after 5 years. Gain Scholarship skill."),
                0, 25, 10, 50, -50f,
                () =>
                {
            ...
            Mercenary.Initialize(new TextObject("{=jzD6JwyXw}Mercenary"),
                new TextObject(
                    "{=3rtWRUzce}You serve as a free mercenary company, roaming around the continent in search of employment. After a long period of joblessness, you find your company in the verge of collapse, with little morale, food and finances."),
                new TextObject(
                    "{=BRoMb4rVS}Start with a mercenary band, in desperate need for plundering gold and food. Party morale reduced for 5 years. Mercenary lifestyle is kickstarted as part of your education."),
                250, 0, 22, 30, 0f,
                () =>
                {
                    var items = Game.Current.ObjectManager.GetObjectTypeList<ItemObject>();
            ...
            Outlaw.Initialize(new TextObject("{=z0v1P29sM}Outlaw"),
                new TextObject(
                    "{=7MdD0jdTu}Lacking in morals, you assemble a party of like-minded brigands, making a living out stealing and plundering. Your efforts, however, have not gone unnoticed by the local authorities."),
                new TextObject(
                    "{=k59YqnupH}Start with a outlaw band, in desperate need for plundering gold and food. Criminal rating does not reduce for 5 years. Outlaw lifestyle is kickstarted as part of your education."),
                50, 2, 15, 50, 0f,
                () =>
                {
                    var items = Game.Current.ObjectManager.GetObjectTypeList<ItemObject>();
            ...
            Caravaneer.Initialize(new TextObject("{=d3bsh65DZ}Robbed Caravaneer"),
                new TextObject(
                    "{=qgP3tvKTx}Your caravan has been recently harassed by criminals - most of your belongings are lost, and certainly all your denars. A few goods, mules and wounded soldiers remain."),
                new TextObject(
                    "{=moXLjDutb}Start with a wounded caravan, some food, mules and goods. Party speed is reduced by 5% for 5 years. Caravaneer lifestyle is kickstarted as part of your education."),
                0, 6, 12, 50, 0f,
                () =>
                {
                    var templates = Game.Current.ObjectManager.GetObjectTypeList<PartyTemplateObject>();
