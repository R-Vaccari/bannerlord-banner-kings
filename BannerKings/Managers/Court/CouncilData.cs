...
namespace BannerKings.Managers.Court
{
    public class CouncilData : BannerKingsData
    {
        ...
        internal override void Update(PopulationData data)
        {
                ...
                if (education.HasPerk(BKPerks.Instance.AugustDeFacto))
                {
                    ...
                    if (Owner == Hero.MainHero)
                    {
                        InformationManager.DisplayMessage(new InformationMessage(
                            new TextObject("{=9m9dFPMZY}You improved relations with {HERO} due to {PERK} lifestyle perk.")
            ...
            /* bool answer = false;
                   InformationManager.ShowInquiry(new InquiryData(new TextObject("{=K1X55kaEj}Council Position Request").ToString(),
                       new TextObject("{=uT2igPB0W}{REQUESTER} requests the position of {POSITION} in your council.")
