    ...
    public class ReligionsManager
    {
        ...
        public void ShowPopup()
        {
            ...
            var elements = Religions.Keys.ToList()
                .Select(religion => new InquiryElement(religion, new TextObject("{=CXBxsAQXM}{RELIGION} - {PIETY} piety").SetTextVariable("RELIGION", religion.Faith.GetFaithName())
                    .SetTextVariable("PIETY", GetPiety(religion))
                    .ToString(), null, true, religion.Faith.GetFaithDescription().ToString()))
                .ToList();

            MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                new TextObject("{=vrWWKoiXW}Your faith").ToString(),
                new TextObject("{=fNbu1Gd4S}You look up to the skies and realize there must be something more. You feel there must be a higher purpose for yourself, and people expect you to defend a certain faith. Upholding your cultural forefathers' faith would be considered most pious. Similarly, following a faith that accepts your culture would be pious, however not as much as your true ancestry. Alternatively, having a completely different faith is possible, though a less walked path. What is your faith?")
                    .ToString(),
                elements, true, 1,
        ...
        public void AddPiety(Religion rel, Hero hero, float piety, bool notifyPlayer = false)
        {
            ...
            {
                MBInformationManager.AddQuickInformation(new TextObject("{=F36PREjPw}{HERO} has recieved {PIETY} piety.")
                    .SetTextVariable("HERO", hero.Name)
