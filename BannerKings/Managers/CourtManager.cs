    ...
    public class CourtManager
    {
            ...
            float maxEffect, bool factor)
        {
            ...
            if (competence != 0f)
            {
                if (!factor)
                {
                    result.Add(maxEffect * competence, new TextObject("{=eCOKqNYgi}Council effect"));
                }
                else
                {
                    result.AddFactor(maxEffect * competence, new TextObject("{=eCOKqNYgi}Council effect"));
                }
            }
        }
        ...
        public void AddHeroToCouncil(CouncilAction action)
        {
            ...
            if (action.ActionTaker == Hero.MainHero)
            {
                MBInformationManager.AddQuickInformation(
                    new TextObject("{=hWtegk8kc}{OWNER} has appointed you as their {POSITION}.")
