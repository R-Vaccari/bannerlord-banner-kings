    ...
    internal class PerksPatches
    {
        ...
        class RegisterPerkPatch
        {
            ...
            private static void MakeStewardPerksBetter()
            {
                if (BannerKingsSettings.Instance.EnableUsefulStewardPerks)
                {
                    ...
                    ChangePerk("StewardFrugal", true, -0.01f,
                        ...
                        SkillEffect.PerkRole.PartyMember);
                    #endregion
                    #region DefaultPerks.Steward.WarriorsDiet (done)    
                    ...
                    #endregion
                    #region DefaultPerks.Steward.SevenVeterans (done)
                    ...
                    #endregion
                    #region DefaultPerks.Steward.StiffUpperLip (not tested)
                    ...
                    #endregion
                    #region DefaultPerks.Steward.Sweatshops (not tested)
                    ...
                    #endregion
                    #region DefaultPerks.Steward.PaidInPromise (not tested)
                    ...
                    ChangePerk("StewardPaidInPromise", true, 0.02f,
                   "Discarded armors are donated to troops for increased experience.\n{VALUE}% bonus experience from donated armors for every 10 levels of steward skill if the hero is the party quartermaster or party leader,\n{VALUE}% bonus experience from donated armors for every 50 levels of steward skill if the hero is a party member.(max +150%)",
                   "Discarded armors are donated to troops for increased experience.\n{VALUE}% bonus experience from donated armors for every 10 levels of steward skill if the hero is the party quartermaster or party leader. (max +150%)"
                   , SkillEffect.PerkRole.PartyLeader, SkillEffect.PerkRole.PartyMember);

                    #endregion
                    #region DefaultPerks.Steward.EfficientCampaigner (not tested)
                    ...
                    #endregion
                    #region DefaultPerks.Steward.GivingHands
                    ...
                    ChangePerk("StewardForeseeableFuture", false, 0.02f,
                  "Discarded weapons are donated to troops for increased experience.\n{VALUE}% bonus experience from donated weapons for every 10 levels of steward skill if the hero is the party quartermaster or party leader,\n{VALUE}% bonus experience from donated weapons for every 50 levels of steward skill if the hero is a party member.(max +150%)",
                  "Discarded weapons are donated to troops for increased experience.\n{VALUE}% bonus experience from donated weapons for every 10 levels of steward skill if the hero is the party quartermaster or party leader. (max +150%)"
                  , SkillEffect.PerkRole.PartyLeader, SkillEffect.PerkRole.PartyMember);

                    ChangePerk("StewardForeseeableFuture", true, 0.01f,
                       "{VALUE}% tariff income in the settlement for every 30 levels of steward skill if the hero is the castle governer,\n{VALUE}% tariff income in the settlement for every 90  levels of steward skill if the hero is a castle owner, \n{VALUE}% tariff income in the settlement for every 120 levels of steward skill if the hero is staying in castle that belongs to his clan.(max +30%)",
                       "{VALUE}% tariff income in the settlement for every 30 levels of steward skill if the hero is the castle governer,\n{VALUE}% tariff income in the settlement for every 90  levels of steward skill if the hero is a castle owner.(max +30%)"
                       , SkillEffect.PerkRole.PartyOwner, SkillEffect.PerkRole.PartyMember);
                    ...
                    ChangePerk("StewardLogistician", false, 1f,
                       "{VALUE} party morale when number of mounts is greater than number of foot troops in your party for every 30 levels of steward skill if hero is the party quartermaster or party leader,\n{VALUE} party morale when number of mounts is greater than number of foot troops in your party for every 150 levels of steward skill if hero is a party member. (max +30)",
                       "{VALUE} party morale when number of mounts is greater than number of foot troops in your party for every 30 levels of steward skill if hero is the party quartermaster or party leader. (max +30)",
                       SkillEffect.PerkRole.PartyLeader, SkillEffect.PerkRole.PartyMember);

                    ChangePerk("StewardLogistician", true, 0.01f,
                      "{VALUE}% tax income in the settlement for every 30 levels of steward skill if the hero is the castle governer,\n{VALUE}% tax income in the settlement for every 90 levels of steward skill if the hero is a castle owner, \n{VALUE}% tax income in the settlement for every 120 levels of steward skill if the hero is staying in castle that belongs to his clan.(max +30%)",
                      "{VALUE}% tax income in the settlement for every 30 levels of steward skill if the hero is the castle governer,\n{VALUE}% tax income in the settlement for every 90 levels of steward skill if the hero is a castle owner.(max +30%)"
                      , SkillEffect.PerkRole.PartyOwner, SkillEffect.PerkRole.PartyMember);
            ...
            private static void ChangePerkRole(string perkId, SkillEffect.PerkRole newRole, bool isSecondary = false)
            {
                ...
                if (perk != null)
                {
                    ChangePerkRole(perk, newRole, isSecondary);
                }
            }
        ...
        class GetMoraleEffectsFromPerksPatch
        {
            static void Postfix(MobileParty party, ref ExplainedNumber bonus)
            {
                ...
                if (party.Party.NumberOfMounts > party.MemberRoster.TotalManCount - num2)
                {
                    ...
                    {
                        PerksHelpers.AddScaledPerkBonus(DefaultPerks.Steward.Logistician, ref bonus, false, party, DefaultSkills.Steward, (float)30, (float)30, (float)150, SkillScale.Both, minValue: 0, maxValue: (float?)30f);
                    }
                    else
                    {
