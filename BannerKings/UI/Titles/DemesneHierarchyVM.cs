...
namespace BannerKings.UI.Titles
{
    public class DemesneHierarchyVM : BannerKingsViewModel
    {
        ...
        public override void RefreshValues()
        {
            ...
            var contractButton = new DecisionElement().SetAsButtonOption(new TextObject("{=CGDsLVSPf}Contract").ToString(),
                () => BannerKingsConfig.Instance.TitleManager.ShowContract(kingdom.Leader,
                    GameTexts.FindText("str_done").ToString()),
                new TextObject("{=4tFHweNCL}Review this kingdom's contract, signed by lords that join it."));
            contractButton.Enabled = allSetup;
            var foundButton = new DecisionElement().SetAsButtonOption(new TextObject("{=2rmLguwm9}Found Kingdom").ToString(),
                ShowFoundKingdom,
                new TextObject("{=4tFHweNCL}Review this kingdom's contract, signed by lords that join it."));
            ...
            var governmentButton = CreateButton(governments,
                governments.Count >= 1
                    ...
                    : null,
                new TextObject("{=kh6UCT2O3}Government").ToString(),
                new TextObject("{=gy9NhB7Ar}Propose a change in government structure, altering the allowed succession forms and aspects of settlement governance. Depending on the government choice, an appropriate succession type will be enforced as well."));
            ...
            var successionButton = CreateButton(successions,
                successions.Count >= 1
                    ...
                    : null,
                new TextObject("{=xEw2zzqnr}Succession").ToString(),
                new TextObject("{=rgKy6CfW8}Propose a change in the realm's succession, altering how the next sovereign is chosen."));
            ...
            var inheritanceButton = CreateButton(inheritances,
                inheritances.Count >= 1
                    ...
                    : null,
                new TextObject("{=weN3ixtJu}Inheritance").ToString(),
                new TextObject("{=DF0OjKcpJ}Propose a change in clan inheritances, that is, who becomes the clan leader once the leader dies."));
            ...
            var genderButton = CreateButton(genderLaws,
                genderLaws.Count >= 1
                    ...
                    : null,
                new TextObject("{=5V15eauSo}Gender Law").ToString(),
                new TextObject("{=RBPOHdPFM}Propose a change in gender laws, dictating whether males and females are viewed equally in various aspects."));
        ...
        private void ShowFoundKingdom()
        {
            ...
            InformationManager.ShowInquiry(new InquiryData(new TextObject("{=RPDF5JEoO}Founding a new Kingdom").ToString(),
                ...
                () =>
                {
                    ...
                    MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                        new TextObject("{=M8gMaA7An}Founding Dukedoms").ToString(),
                        new TextObject("{=Kt962ViVF}Select up to 3 dukedoms that will compose your kingdom. The kingdom's contract will follow the first dukedom's contract. Dukedom titles from other clans in the faction may be included as well.").ToString(),
                        duchies,
                        true,
                        3,
                        GameTexts.FindText("str_done").ToString(),
        ...
        private DecisionElement CreateButton(List<InquiryElement> options, BKContractDecision decision, string law, TextObject hint)
        {
            return new DecisionElement()
                ...
                {
                    var description = new TextObject("{=jae89RXTt}Select a {LAW} to be voted on. Starting an election costs {INFLUENCE} influence.");
        ...
        private List<InquiryElement> GetGenderLaws()
        {
            ...
            foreach (var type in BannerKingsConfig.Instance.TitleManager.GetGenderLawTypes())
            {
                if (kingdom != null && type != title.contract.GenderLaw)
                {
                    ...
                    var text = new TextObject("{=24Mmf8Rif}{LAW} - ({SUPPORT}% support)");
