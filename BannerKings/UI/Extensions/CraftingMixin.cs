...
namespace BannerKings.UI.Extensions
{
    ...
    public class CraftingMixin : BaseViewModelMixin<CraftingVM>
    {
        ...
        [DataSourceProperty] public string ArmorText => new TextObject("{=9riZDG676}Craft").ToString();
        ...
        public override void OnRefresh()
        {
            ...
            HoursSpentText = new TextObject("{=RtBnqjBB5}Hours spent for all actions: {HOURS} hours.")
        ...
        public void ExecuteMainActionBK()
        {
            ...
            else
            {
                ...
                {
                    MBInformationManager.AddQuickInformation(new TextObject("{=r36GFYxzM}{HERO} has botched {ITEM}!")
                    ...
                    (item.HasArmorComponent && item.ArmorComponent.ItemModifierGroup != null))
                {
                    ...
                    {
                        qualityText =
                            new TextObject("{=RSeTH8GRy} with {QUALITY} quality").SetTextVariable("QUALITY", modifier.Name);
                        element.SetModifier(modifier);
                    }
                }

                MBInformationManager.AddQuickInformation(new TextObject("{=CC9KeHGm0}{HERO} has crafted {ITEM}{QUALITY}.")
                        .SetTextVariable("HERO", crafting.CurrentCraftingHero.Hero.Name)
                        .SetTextVariable("ITEM", item.Name)
