...
namespace BannerKings.UI.Extensions
{
    ...
    public class CraftingMixin : BaseViewModelMixin<CraftingVM>
    {
        ...
        [DataSourceProperty] public string ArmorText => new TextObject("{=YJejZRSww}Craft").ToString();
        ...
        public override void OnRefresh()
        {
            ...
            HoursSpentText = new TextObject("{=714WmRsrM}Hours spent for all actions: {HOURS} hours.")
        ...
        public void ExecuteMainActionBK()
        {
            ...
            else
            {
                ...
                {
                    MBInformationManager.AddQuickInformation(new TextObject("{=m5Yi0GrwZ}{HERO} has botched {ITEM}!")
                    ...
                    (item.HasArmorComponent && item.ArmorComponent.ItemModifierGroup != null))
                {
                    ...
                    {
                        qualityText =
                            new TextObject("{=1nJvPh5Bn} with {QUALITY} quality").SetTextVariable("QUALITY", modifier.Name);
                        element.SetModifier(modifier);
                    }
                }

                MBInformationManager.AddQuickInformation(new TextObject("{=BQEfqf4vO}{HERO} has crafted {ITEM}{QUALITY}.")
                        .SetTextVariable("HERO", crafting.CurrentCraftingHero.Hero.Name)
                        .SetTextVariable("ITEM", item.Name)
