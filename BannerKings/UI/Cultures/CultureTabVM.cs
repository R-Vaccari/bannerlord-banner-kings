    ...
    public class CultureTabVM : BannerKingsViewModel
    {
        ...
        public override void RefreshValues()
        {
            ...
            if (innovationData != null)
            {
                ...
                if (innovationData.CulturalHead != null)
                {
                    CultureInfo.Add(new InformationElement(new TextObject("{=AoqhWZU9}Cultural Head:").ToString(),
                        ...
                        new TextObject("{=eTv4mF9d}The Cultural Head is the most proeminent lord or lady within a culture. They are able to determine a culture's Fascination, which is an innovation that develops faster. They are also more influential as a family. Usually this position is occupied by a ruler.").ToString()));
                }
                ...
                if (innovationData.Fascination != null)
                {
                    CultureInfo.Add(new InformationElement(new TextObject("{=eLLt37Yx}Cultural Fascination:").ToString(),
                        ...
                        new TextObject("{=4uTRjRsY}A Cultural Fascination is an innovation that takes precedence over others. This innovation will receive more research points, developing faster while slowing down other innovations. The Cultural Head is responsible for deciding the fascination.\n{NAME}:\n{DESCRIPTION}")
                ...
                CultureInfo.Add(new InformationElement(new TextObject("{=lIcoh5Mr}Years Remaining:").ToString(),
                    new TextObject("{=bYPWBS7f}{COUNT} years")
                    .SetTextVariable("COUNT", (days / (float)CampaignTime.DaysInYear).ToString("0.0"))
                    .ToString(),
                    new TextObject("{=4NPQNPBr}How many years remanining until all innovations for this Era are done, approximately.").ToString()));
            ...
            CultureInfo.Add(new InformationElement(new TextObject("{=VRbXbsPE}Population:").ToString(),
                population.ToString(),
                new TextObject("{=MLVVQm5u}The approximate number of people that follow this culture, across the continent.").ToString()));
        ...
        private void ChangeFascination()
        {
            if (innovationData != null)
            {
                ...
                MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
                    ...
                    new TextObject("{=u1gHDZih}The cultural fascination is an innovation that progresses faster than others. As Cultural Head, you can determine the fascination. Changing fascinations incurs no type of cost.")
                        .ToString(),
                    elements, true, 1,
                    GameTexts.FindText("str_done").ToString(), string.Empty,
                    delegate (List<InquiryElement> x)
