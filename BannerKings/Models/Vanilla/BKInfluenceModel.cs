    ...
    public class BKInfluenceModel : DefaultClanPoliticsModel
    {
        ...
        public override ExplainedNumber CalculateInfluenceChange(Clan clan, bool includeDescriptions = false)
        {
                    ...
                    BannerKingsConfig.Instance.PopulationManager.IsSettlementPopulated(settlement))
                {
                    ...
                    if (BannerKingsConfig.Instance.AI.AcceptNotableAid(clan, data))
                    {
                        foreach (var notable in data.Settlement.Notables)
                        {
                            ...
                            {
                                baseResult.Add(-1f,
                                    new TextObject("{=sXcfds7Bo}Aid from {NOTABLE}").SetTextVariable("NOTABLE", notable.Name));
                            }
                        }
                    }
            ...
            if (BannerKingsConfig.Instance.CourtManager != null)
            {
                ...
                if (position != null)
                {
                    baseResult.Add(position.IsCorePosition(position.Position) ? 1f : 0.5f,
                        new TextObject("{=JcT7c7hzg}Councillor role"));
                }
            }
            ...
            if (i > 0)
            {
                ...
                if (finalSupport != 0f)
                {
                    baseResult.AddFactor(finalSupport, new TextObject("{=TOSZ5PDRn}Overall notable support"));
                }

                if (finalAutonomy != 0f)
                {
                    baseResult.AddFactor(finalAutonomy, new TextObject("{=XmdfGjcEc}Overall settlement autonomy"));
                }
            }
        ...
        public ExplainedNumber CalculateSettlementInfluence(Settlement settlement, PopulationData data)
        {
            ...
            if (villageData != null)
            {
                ...
                if (manor > 0)
                {
                    settlementResult.AddFactor(manor == 3 ? 0.5f : manor * 0.15f, new TextObject("{=yCr3ZYXFQ}Manor"));
                }
            }
